...
function totalLicks(env) {
  ...
  var conditionTotal = 0,
      toughestConditionRank = -999999,
      toughCondition= "Nothing",
      message = "",
      somethingBadHappened = false;

  for(var condition in env){
    conditionTotal+= env[condition];
    if( env[condition] > 0) somethingBadHappened = true;
    if(env[condition] > toughestConditionRank){

      toughestConditionRank = env[condition];
      toughCondition = condition;
    }
  }
  message = (conditionTotal+ 252 > 252 || somethingBadHappened) ? "It took " + (conditionTotal + 252) + " licks to get to the tootsie roll center of a tootsie pop. The toughest challenge was " + toughCondition + "." : "It took " + (conditionTotal + 252) + " licks to get to the tootsie roll center of a tootsie pop." ;
  return message
}
